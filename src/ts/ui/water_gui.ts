...
export default class WaterGUI extends RoadGUI {
    ...
    constructor(private tensorField: TensorField,
                ...
                folderName: string,
                redraw: () => void) {
        super(params, integrator, folderName, redraw);
    ...
    get river(): Vector[] {
        return this.streamlines.riverPolygon.map(v => this.worldToScreen(v.clone()));
    }

    get secondaryRiver(): Vector[] {
        return this.streamlines.riverSecondaryRoad.map(v => this.worldToScreen(v.clone()));
    }

    get coastline(): Vector[] {
        // Use unsimplified noisy streamline as coastline
        // Visual only, no road logic performed using this
        return this.streamlines.coastline.map(v => this.worldToScreen(v.clone()));
    }

    get seaPolygon(): Vector[] {
        return this.streamlines.seaPolygon.map(v => this.worldToScreen(v.clone()));
    }
